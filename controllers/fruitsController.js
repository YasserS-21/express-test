...
fruits.get("/search", (request, response) => {
    ...
    const fruitsWithCalories = fruitsData.filter(fruit => Number(fruit.nutritions.calories) <= Number(calories))
...
fruits.get("/:fruitName", (request, response) => {
    ...
    const foundFruit = fruitsData.filter(fruit => fruit.name.toLowerCase() === fruitName.toLowerCase())
